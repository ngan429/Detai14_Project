...
from my_app.models import UserCreateDatabase, UserCreateTable
...
class UserProfileView(BaseView):
    ...
    def index(self):
        user = Users.query.filter_by(Id=current_user.Id, IsAdmin=0, Active=1).first()
...
class UserManageTableView(BaseView):
    ...
    def index(self):
        ...
        # self._template_args["my_database"] = userCreateDatabase
...
class SubmitUserManageTableView(BaseView):
    @expose('/', methods=["POST"])
    def index(self):
        getNameDatabase = (request.form.get('choose_database'))
        if request.method == "POST":
            getNamTable = ""
            if request.method == "POST":
                getNamTable = request.form.get('nameTable')

                url = "https://sqs.us-east-1.amazonaws.com/126581837666/testQueue"
                headers = {"Content-Type": "application/x-www-form-urlencoded"}

                payload = {
                    "name_table": getNamTable
                }
                payload = quote(str(payload))
                params = {
                    'Action': 'SendMessage',
                ...
                userCreateTable.Id = current_user.Id
                userCreateTable.NameDatabase = getNameDatabase
                userCreateTable.NameTable = getNamTable
                db.session.add(userCreateTable)
                try:
                    db.session.commit()
...
class AddColumnTable(ModelView):
    ...
    def index(self):
        ...
        listQuery = []
        while (request.form.get('nameTable' + str(i)) != "" and request.form.get('nameTable' + str(i)) is not None) or (
                request.form.get('chooseTypeColumn' + str(i)) != "" and request.form.get(
                'chooseTypeColumn' + str(i)) is not None):
...
class RenameColumnTableView(ModelView):
    ...
    def details(self):
        ...
        if request.method == "POST":
            query = "ALTER TABLE " + name_database + "." + name_table + " RENAME COLUMN " + column + " TO " + request.form.get(
                'newColumnName')
...
userView.add_view(
    DetailDatabaseView(Users, db.session, name="Detail Database", url='/user/database', endpoint='_detailDatabase'))
userView.add_view(
    DetailTableView(Users, db.session, name="Detail Table", url='/user/database', endpoint='_detailTable'))
userView.add_view(DetailDataTableView(Users, db.session, name="Detail Data Table", url='/user/database/table/detail',
                                      endpoint='_detailDataTable'))
userView.add_view(EditColumnTableView(Users, db.session, name="Edit Column Table", url='/user/database/table',
                                      endpoint='_editColumnTable'))
userView.add_view(AddColumnTable(Users, db.session, name="add_column_table", url='/user/database/table/column',
                                 endpoint='_addColumnTable'))
userView.add_view(
    RenameColumnTableView(Users, db.session, name="Rename Column Table", url='/user/database/table/rename',
                          endpoint='_renameColumnTable'))
userView.add_view(SubmitDropColumnTable(Users, db.session, name="drop_column_table", url='/user/database/table/drop',
                                        endpoint='_dropColumnTable'))
userView.add_view(AddDataTable(name="add_data_table", url='/user/add-data-table', endpoint='_addDataTable'))
userView.add_view(EditDataTableView(Users, db.session, name="Edit Data Table", url='/user/edit-data-table',
...
userView.add_view(
    SubmitDeleteDataTable(Users, db.session, name="submit_delete_data_table", url='/user/delete-data-table/submit',
                          endpoint='_deleteDataTable'))
userView.add_view(
    SubmitDropTable(Users, db.session, name="submit_drop_table", url='/user/drop-table/submit', endpoint='_dropTable'))
userView.add_view(SubmitDropDatabase(Users, db.session, name="submit_drop_database", url='/user/drop-database/submit',
                                     endpoint='_dropDatabase'))
userView.add_view(
    AllDetailsDatabase(Users, db.session, name="Detail All Data Of Database", url='/user/all-details-database',
                       endpoint='_allDetailsDatabase'))
