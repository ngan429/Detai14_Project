import sys
import pymysql
from my_app.models import Users, Datatype, UserCreateDatabase
...
#             # query = "SELECT * FROM ManageUser.UserCreateDatabase;"
#             cur.execute(query)
#             data = cur.fetchall()
#         conn.commit()
...
def get_all_database(username):
    ...
    try:
        with conn.cursor() as cur:
            query = "SELECT Name FROM ManageUser4.UserCreateDatabase;"
            cur.execute(query)
            data = cur.fetchall()
        conn.commit()
    except Exception as e:
...
def CountRowTable(namedatabase, nametable):
    ...
    try:
        with conn.cursor() as cur:
            ...
            query = "SELECT COUNT(*) FROM ManageUser.InfoAddRowDatabase;"
            cur.execute(query)
            data = cur.fetchall()
        conn.commit()
    except Exception as e:
...
def SubmitAddTable(data):
    url = "https://sqs.us-east-1.amazonaws.com/799888255921/queueAddDataTable"
